...
class QwenTagPredictor:
    ...
    def predict_tags(self, text: str, available_tags: list[str]) -> list[str]:
        ...
        prompt = (
            f"Прочитай текст и выбери от 1 до 5 тегов, которые наилучшим образом его описывают. "
            f"Допустимы только следующие теги: {tag_list}. "
            f"Верни только список выбранных тегов, разделённый запятыми, без лишних слов.\n\n"
            f"Текст:\n{text.strip()}"
        )
